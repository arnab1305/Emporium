...
export class CheckoutComponent implements OnInit {
  ...
  constructor(private _auth: AuthService, private _cart: CartService ,private _prod: ProductService) {
  ...
  submitCheckout() {
    ...
    for(let i = 0; i<this.cartData.products.length;i++){
      ...
      setTimeout(() => {
        this._prod
          .orderData({
            id: this.cartData.products[i].id,
            quantity: this.cartData.products[i].quantity*this.cartData.products[i].price
          })
          .subscribe(
            (res: any) => {
              console.log(res);
